...
const parseHealthCheckEntry = (entry: object): NewEntry => {
    if ('type' in entry && isEntryType(entry.type) && entry.type === EntryType.HealthCheck && 'healthCheckRating' in entry && !isNaN(Number(entry.healthCheckRating)) && isHealthCheckRating(Number(entry.healthCheckRating))) {
        return ({
            ...parseBaseEntry(entry),
            type: entry.type,
